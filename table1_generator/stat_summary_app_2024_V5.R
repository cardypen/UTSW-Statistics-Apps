...
ui <- navbarPage(theme = shinytheme("cerulean"), title = "Basic Statistics Calculator",
                 ...
                 tabPanel(title = "Analysis",
                          ...
                          sidebarLayout(
                            sidebarPanel( 
                              ...
                              # Input: Select row or column percentage ----
                              radioButtons("row_or_col", "Choose to display row or column percentages:",
                                           inline = FALSE,
                                           choices = c("Column", 
                                                       "Row")),
                 ...
                 # Third panel to display pairwise results ----
                 tabPanel(title = "Pairwise Comparisons",
                          
                          sidebarLayout(
                            sidebarPanel( 
                              checkboxGroupInput("metric2", "Choose metrics to include in the table:",
                                                 choices = c("Mean ± SD","Median (Q1,Q3)"), selected = "Mean ± SD"),
                              
                              # Input: Select whether to include missing ----
                              radioButtons("missing2", "Choose to include number of values missing:",
                                           inline = FALSE,
                                           choices = c("do not incude missing",
                                                       "include missing")),
                              
                              radioButtons("test_con2", "Choose a test for continuous variables:",
                                           inline = FALSE,
                                           choices =  c("Wilcoxon Rank Sum Test",
                                                        "Linear Model with Contrasts")),
                              
                              # Input: Select test for categorical with two levels ----
                              radioButtons("test_cat2", "Choose a test for catagorical variables:",
                                           inline = FALSE,
                                           choices = c("Fisher Exact Test",
                                                       "Chi Square Test")),
                              
                              radioButtons("pairwise_padj_method", "Choose a multiple comparison p-value adjustment method:",
                                           inline = FALSE,
                                           choices = c("Holm",
                                                       "None"))
                            ...
                            mainPanel(
                              ...
                              #Help text statement ----
                              helpText("Results may take a few seconds to load."),
                              
                              
                              # Output results table with loader ----   
                              
                              withSpinner(gt::gt_output('table_pairwise')),
...
server <- function(session, input, output) {
  ...
  gtObject <- reactive({
    ...
    gts_object <- gtsummary::tbl_summary(selectedData()[input$mytable2_rows_all,], by=if (input$selectgrp == "None") NULL else input$selectgrp,
                                         ...
                                         percent = case_when(input$row_or_col == "Column"~"column", 
                                                             input$row_or_col == "Row"~"row"))
  ...
  output$resultstxt<-renderUI({
    ...
    # make pairwise table
    gtObject_pairwise <- reactive({
      req(input$selectcon, input$selectgrp)
      gts_object <- gtsummary::tbl_summary(selectedData()[input$mytable2_rows_all,], by=if (input$selectgrp == "None") NULL else input$selectgrp,
                                           type = c(all_continuous() ~ "continuous2",
                                                    input$selectcat ~ "categorical"),
                                           statistic = case_when( "Mean ± SD" %!in% input$metric2 ~ list(
                                             all_continuous() ~ "{median} ({p25}, {p75})",
                                             all_categorical() ~ "{p}% ({n}/{N})"),
                                             "Median (Q1,Q3)" %!in% input$metric2 ~ list(
                                               all_continuous() ~ "{mean} ± {sd}",
                                               all_categorical() ~ "{p}% ({n}/{N})"),
                                             .default = list(all_continuous() ~ c("{mean} ± {sd}","{median} ({p25}, {p75})"),
                                                             all_categorical() ~ "{p}% ({n}/{N})"
                                             )),
                                           missing = case_when(input$missing2 == "do not incude missing"~"no",
                                                               input$missing2 == "include missing"~"always"),
                                           missing_text = "Missing")
      ...
      gts_object2<- if ("None" %!in% input$selectgrp) gts_object %>% add_pairwise(
        continuous_method = ifelse(input$test_con2=="Wilcoxon Rank Sum Test", "wilcox.test", "lm_emmeans"),
        categorical_method = ifelse(input$test_cat2== "Fisher Exact Test", "fisher", "chisq"),
        p.adjust.method = ifelse(input$pairwise_padj_method == "Holm", "holm", "none")
        ) else gts_object
    ...
    output$table_pairwise <- render_gt({ 
      req(input$selectcon)
      gtObject_pairwise() })
